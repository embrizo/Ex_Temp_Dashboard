...
export default function AnalysisPage() {
  ...
  const CustomTooltip = ({ active, payload }) => {
    ...
    return (
      <div style={{ background: 'var(--color-bg-2)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', padding: 'var(--space-3)', minWidth: 180 }}>
        ...
        <div style={{ fontSize: 'var(--text-lg)', fontWeight: 700, color: 'var(--color-cyan)' }}>{d?.val?.toFixed(2)} {unit}</div>
  ...
  const hourlyData = useMemo(() => {
    const byHour = Array(24).fill(null).map((_, h) => {
      ...
      const avg = hrs.reduce((a, b) => a + b.val, 0) / hrs.length;
      ...
      <div className="page-content">
        ...
        <div style={{ paddingTop: 'var(--space-6)', marginBottom: 'var(--space-6)', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 'var(--space-4)' }}>
          <div>
            <div className="eyebrow" style={{ marginBottom: 'var(--space-2)' }}>
              <Icon size={12} /> Deep-Dive Analysis
            </div>
        ...
        <div className="glass-card chart-panel card-glow-cyan" style={{ marginBottom: 'var(--space-4)' }}>
          <div className="chart-header">
            <div>
              <div className="chart-title">Detailed {metricName} Trend</div>
          ...
          <ResponsiveContainer width="100%" height={380}>
            <LineChart data={chartData} margin={{ top: 5, right: 30, left: 0, bottom: 5 }}>
              ...
              <Line type="monotone" dataKey="val" stroke="var(--color-cyan)" strokeWidth={1.5} dot={false} activeDot={{ r: 4, fill: 'var(--color-cyan)' }} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-4)', marginBottom: 'var(--space-4)' }}>
          ...
          <div className="glass-card chart-panel card-glow-blue">
            <div className="chart-header">
              <div>
                <div className="chart-title">{metricName} Distribution</div>
            ...
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={bandData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
                <XAxis dataKey="range" tick={{ fontSize: 10, fill: 'var(--color-text-3)' }} stroke="var(--color-text-3)" tickFormatter={(v) => stats.type === 'Temperature' ? `${v}°` : v} />
          ...
          <div className="glass-card chart-panel card-glow-purple" style={{ '--color-purple-glow': 'rgba(139,92,246,0.12)' }}>
            ...
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={hourlyData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                ...
                <YAxis tick={{ fontSize: 10, fill: 'var(--color-text-3)' }} stroke="var(--color-text-3)" width={36} tickFormatter={(v) => stats.type === 'Temperature' ? `${v}°` : v} domain={['auto', 'auto']} />
                <Tooltip
                  formatter={(val) => [`${val} ${unit} avg`, metricName]}
                  contentStyle={{ background: 'var(--color-bg-2)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', fontSize: 12, color: 'var(--color-text)' }}
                  itemStyle={{ color: 'var(--color-text)' }}
        ...
        <div className="glass-card chart-panel">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 'var(--space-3)' }}>
            {[
              ...
              { label: 'Mean (μ)', value: `${(filtered.reduce((a,b)=>a+b.val,0)/filtered.length).toFixed(3)} ${unit}`, color: 'var(--color-cyan)' },
              { label: 'Minimum', value: `${Math.min(...filtered.map(r=>r.val)).toFixed(2)} ${unit}`, color: 'var(--color-blue-light)' },
              { label: 'Maximum', value: `${Math.max(...filtered.map(r=>r.val)).toFixed(2)} ${unit}`, color: 'var(--color-red)' },
              { label: 'Std Dev (σ)', value: (() => { const m=filtered.reduce((a,b)=>a+b.val,0)/filtered.length; return `${Math.sqrt(filtered.reduce((a,b)=>a+(b.val-m)**2,0)/filtered.length).toFixed(3)} ${unit}`; })(), color: 'var(--color-purple-light)' },
              { label: 'High Threshold', value: `${currentHigh} ${unit}`, color: 'var(--color-red)' },
              { label: 'Low Threshold', value: `${currentLow} ${unit}`, color: 'var(--color-blue-light)' },
